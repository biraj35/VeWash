...
import com.eliteinfotech.vewash.ServiceProvider.OrderSummaryAdapter;
...
public class YearlySummaryActivity extends AppCompatActivity {
    ...
    com.eliteinfotech.vewash.ServiceProvider.OrderSummaryAdapter userOrderAdapter;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mToolbar.setTitle("Yearly Summary");
        ...
        txLabel = findViewById(R.id.txtLabel);
        txLabel.setText("Year");
    ...
    ArrayList<SummaryInfo> summarize() {
        ...
        for (int i = 0; i < orderLst.size(); i++) {
            if (!dlist.contains(DateUtils.convertToString(orderLst.get(i).getCreatedDate(), "yyyy")))
                dlist.add(DateUtils.convertToString(orderLst.get(i).getCreatedDate(), "yyyy"));
        }
    ...
    String getCount(String date) {
        ...
        dl.clear();
        for (int i = 0; i < orderLst.size(); i++) {
            if (DateUtils.convertToString(orderLst.get(i).getCreatedDate(), "yyyy").equals(date))
        ...
        try {
            ...
            databaseReference.addListenerForSingleValueEvent(new ValueEventListener() {
                ...
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    ...
                    if (orderLst.size() > 0) {
                        userOrderAdapter = new OrderSummaryAdapter(summarize(), YearlySummaryActivity.this);
                ...
                public void onCancelled(@NonNull DatabaseError databaseError) {
                    Toast.makeText(YearlySummaryActivity.this, databaseError.getMessage(), Toast.LENGTH_SHORT).show();
                    UtilityHelper.hideView(progressBar);
                    mNoData.setText(databaseError.getMessage());
                }
            });
        } catch (Exception ex) {
            Toast.makeText(YearlySummaryActivity.this, ex.getMessage(), Toast.LENGTH_SHORT).show();
            UtilityHelper.hideView(progressBar);
